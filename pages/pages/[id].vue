			...
			<div v-else>
				...
				<ul role="list" class="max-w-full overflow-x-hidden space-y-2 my-4 pt-1">
					<li v-for="item in items" :key="item.id">
						<list-item v-bind="item" v-model:price="item.price" v-model:count="item.count"
							@update:price="storedItems.save()" @update:count="storedItems.save()">
							{{ item.text }}
						</list-item>
					</li>
					<li>
						<app-button v-if="!newItem" class="w-full border-none underline" @click="newItem = true">
							Nuevo elemento
						</app-button>
